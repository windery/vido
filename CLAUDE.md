...
- **Ctrl+V 可视块模式**：锚点=按下时光标，移动键（hjkl/w/b/e/0/$/gg/G）扩展矩形选区（半透明绿底覆盖层实时高亮）；`x`/`d` 删块（内容入内部缓冲+系统剪贴板）、`y` 只复制、`c` 删块后进入插入、**`p`/`P` 用粘贴内容替换块**（vim 语义：系统剪贴板优先、回退内部 yank，空文本=仅删除）、`Esc` 只退出块模式；未绑定键先退出块再按普通键处理；切换任务/退出导航自动清理选区。
...
- **网格任务格内联标记**：优先级 `!!!`/`!!`/`!`（ANSI 色，与列表一致）+ 日程时刻 `◷10:00`（从日程数据提取，过期日程也显示）；选中/翻页/切粒度时高亮日格滚入视口
