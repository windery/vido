...
- **月网格只渲染当月实际天数**（1 号对齐其星期列，上/下月日期不占格）；month/week 视图均按此渲染
...
- **week 视图**：当月全部天数展示，仅当前周（切换范围）正常显示，范围外日期置灰；翻到下月的周时显示月份随之切换（anchor 月份 = 活跃周所在月）
- `Enter` 网格开当日详情 / 详情开任务；`Esc` 详情回网格 / 网格退出；`H/L` 切粒度（day/week/month，仅日历内）；`[ ]` 翻页（**以焦点日为基准**：同一天号、目标月天数不足收敛月末，绝不被锚点日劫持）
