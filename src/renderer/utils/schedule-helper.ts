...
export function parseScheduleFromString(timeStr: string): TaskSchedule | null {
  ...
  // 尝试解析日期时间格式
  // 支持 YYYY-MM-DD HH:MM:SS 和 YYYY-MM-DD HH:MM 格式
  if (/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}(:\d{2})?$/.test(trimmed)) {
    // 如果没有秒，自动添加 :00
    const dateTime = trimmed.includes(':') && trimmed.split(':').length === 2 
      ? `${trimmed}:00` 
      : trimmed;
    return createSpecificDateTimeSchedule(dateTime);
  }
  ...
  if (trimmed.includes('-') || trimmed.includes('到')) {
    ...
    if (parts.length === 2) {
      ...
      if (
        ...
        // 如果没有秒，自动添加 :00
        const startDateTime = start.includes(':') && start.split(':').length === 2 
          ? `${start}:00` 
          : start;
        const endDateTime = end.includes(':') && end.split(':').length === 2 
          ? `${end}:00` 
          : end;
        return createDateTimeRangeSchedule(startDateTime, endDateTime);
      }
